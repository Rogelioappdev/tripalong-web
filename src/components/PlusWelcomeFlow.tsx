...
import { pollForPlus } from '@/lib/subscription'
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 gap-8 px-8 text-center">
      ...
      <div>
        ...
        >
          Everything's unlocked. Go find your next trip.
        </motion.p>
      </div>
...
function PlusSlides({ userId, onDone }: { userId: string; onDone: (profile: UserProfile | null) => void }) {
  ...
  const handleDone = async () => {
    ...
    // Poll (not a single fetch) — RevenueCat's webhook writing subscription_tier
    // to Supabase can lag a beat behind the client-side purchase confirmation,
    // so a one-shot getProfile() can still read stale 'free' here. By the time
    // the user has clicked through 4 slides this has almost always caught up.
    const confirmed = await pollForPlus(userId).catch(() => null)
    onDone(confirmed)
  }
...
interface PlusWelcomeFlowProps {
  userId: string
  // Called once with the server-confirmed profile (subscription_tier really
  // is plus/pro by now, not just an optimistic guess) — or null if the user
  // skipped the slides early. Callers should commit this as the new source of
  // truth (feed profile, local profile, etc.) rather than trusting whatever
  // optimistic flip happened right after the purchase resolved.
  onDone: (profile: UserProfile | null) => void
}

// Full-screen "Welcome to Plus" animation + 4-slide feature tour, shown right
// after a purchase succeeds. While the user is clicking through it, we poll
// the server in the background (see PlusSlides.handleDone) so that by the
// time they land back in the app, subscription_tier is guaranteed correct
// everywhere — not just wherever an optimistic local flip happened to reach.
export function PlusWelcomeFlow({ userId, onDone }: PlusWelcomeFlowProps) {
  const [phase, setPhase] = useState<'unlock' | 'slides'>('unlock')
  const [travelImages, setTravelImages] = useState<string[]>([])

  useEffect(() => {
    getTravelImages(12).then(setTravelImages)
  }, [])
    ...
    <div className="fixed inset-0 z-[100]" style={{ backgroundColor: '#050505' }}>
      {phase === 'unlock' && (
        <motion.div
          ...
          style={{ position: 'absolute', inset: 0 }}
        >
          <UnlockAnimation onComplete={() => setPhase('slides')} images={travelImages} />
        </motion.div>
      )}
      {phase === 'slides' && (
        <motion.div
          ...
          style={{ position: 'absolute', inset: 0 }}
        >
          <PlusSlides userId={userId} onDone={onDone} />
        </motion.div>
      )}
    </div>
  )
