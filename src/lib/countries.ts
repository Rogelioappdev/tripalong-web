...
// name → flag emoji lookup (for displaying stored values)
export const COUNTRY_FLAG_MAP: Record<string, string> = Object.fromEntries(
  COUNTRIES.map(c => [c.name, countryFlag(c.code)])
)

export function getFlag(name: string): string {
  return COUNTRY_FLAG_MAP[name] ?? ''
}
